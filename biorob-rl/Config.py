...
def getEnv(NAME,PORT,ACTION_DIM,ACTION_REPEAT,RANDOM_SEED,WORLD):
    ...
    register(
        ...
        entry_point='gym.envs.webots:RegisBrain1',
        kwargs=dict(
...
class Config:
    ...
    def __init__(self,game="Regis-v",args=None):
        ...
        self.UPDATE_GLOBAL_ITER = 1 # Has te be set to one and the episode has to last longer than the
                                    # than action_repeat
        ...
        self.env = getEnv(self.GAME,self.INIT_PORT,args["action_dim"],self.ACTION_REPEAT,self.RANDOM_SEED, args["world"])
        ...
        self.ACTION_BOUND = [self.env.action_space.low, self.env.action_space.high] # TODO check if used
        self.ACTION_GAP = self.env.action_space.high - self.env.action_space.low # TODO check if used.
