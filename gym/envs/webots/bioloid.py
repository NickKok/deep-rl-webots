"""
Gym Environment for the Bioloid robot of the
...
class Bioloid(webots_env.WebotsEnv):
    ...
    def __init__(self):
        webots_env.WebotsEnv.__init__(self, ROBOT_CFG, 2)
    ...
    def _step(self,action):
        ...
        if(len(self.model.data[optimizeFor]) == 0): # if first step
            ...
            target_vec_forward = np.array([1,0,0])
            target_vec_forward = target_vec_forward/np.linalg.norm(target_vec_forward)
            target_vec_upward = np.array([0,1,0])
            target_vec_upward = target_vec_forward/np.linalg.norm(target_vec_forward)
            # 2) Do action
            self.do_simulation(action, self.frame_skip)
            ...
            ndiffHead = head_vec_after-head_vec_before
            ...
            reward_forward = np.dot(target_vec_forward,ndiffHead)
            reward_leg = 0*np.dot(target_vec_forward,ndiffLeg)
            reward_upward =  np.dot(target_vec_upward,ndiffHead)
        ...
        reward = 0*max(0,reward_forward)+reward_upward+reward_survive
