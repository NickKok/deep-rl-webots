...
class PolicyEstimator():
  ...
  def __init__(self, num_outputs, reuse=False, trainable=True):
    ...
    # Our input are 4 Frames of shape 47, 1 each
    self.states = tf.placeholder(shape=[None, 47, 4], dtype=tf.float32, name="X")
    # The TD target value
    self.targets = tf.placeholder(shape=[None], dtype=tf.float32, name="y")
    # Policy action id of which action was selected
    #self.actions = tf.placeholder(shape=[None], dtype=tf.float32, name="actions")

    # Normalize TODO TO BE CHANGED
    ...
    with tf.variable_scope("policy_net"):
      ...
      dense1 = dense(fc1, 200, None, w_i, None, activation=tf.nn.relu6)
      with tf.variable_scope('mu'):
        #self.mu = dense(dense1, self.num_outputs, None, w_i, None, activation=tf.nn.tanh)
        # Linear activation like in the article of A3C
        linear_W = tf.Variable(tf.truncated_normal([prev_units, self.num_outputs], …))
        linear_b = tf.Variable(tf.zeros([self.num_outputs]))
        self.mu = tf.matmul(dense1, linear_W) + linear_b

      with tf.variable_scope('sigma'):
        self.sigma = dense(dense1, self.num_outputs, None, w_i, None, activation=tf.nn.softplus)


      self.action_normal_dist = tf.contrib.distributions.Normal(self.mu, self.sigma)
      self.actions = self.action_normal_dist.sample(1)

      # We add entropy to the loss to encourage exploration
      self.entropy = -0.5*tf.reduce_sum(tf.log(2*np.pi*self.sigma**2), 1, name="entropy")
      self.entropy_mean = tf.reduce_mean(self.entropy, name="entropy_mean")

      # Get the predictions for the chosen actions only
      #gather_indices = tf.range(batch_size) * tf.shape(self.probs)[1] + self.actions
      #self.picked_action_probs = tf.gather(tf.reshape(self.probs, [-1]), gather_indices)
      self.picked_action_probs = self.probs

      self.losses = - (tf.log(tf.reduce_sum(self.picked_action_probs)) * self.targets + 1e-4 * self.entropy)
