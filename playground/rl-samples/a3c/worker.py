...
class Worker(object):
  ...
  def run(self, sess, coord, t_max):
    with sess.as_default(), sess.graph.as_default():
      # Initial state
      self.state = np.stack([self.env.reset()] * 4, axis=1)
      try:
  ...
  def _policy_net_predict(self, state, sess):
    ...
    preds = sess.run(self.policy_net.actions, feed_dict)
    return preds[0]
  ...
  def run_n_steps(self, n, sess):
    ...
    for _ in range(n):
      # Take a step
      action = self._policy_net_predict(self.state, sess)
      #action_probs = self._policy_net_predict(self.state, sess)
      #action = np.random.choice(np.arange(len(action_probs)), p=action_probs)
      #print(action)
      next_state, reward, done, _ = self.env.step(action)
      next_state = np.append(self.state[:,1:], np.expand_dims(next_state, 1), axis=1)
  ...
  def update(self, transitions, sess):
    ...
    for transition in transitions[::-1]:
      # Discounted reward. We start from the end of transitions and always add back a bit
      # of the next reward in time. Which means that a reward later as an effect before
      reward = transition.reward + self.discount_factor * reward
      # We calculate the difference between the reward we got and the one predicted by the value_network
